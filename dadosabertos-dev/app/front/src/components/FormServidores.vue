...
export default {
  ...
  methods: {
    carregarItemsNivelAgregacao() {
      return Store.state.items_nivel_agregacao_servidores
    },
    ...
    atualizarURI(){
      ...
      let baseURI = '/api/v1';
      let fonte = '/servidor';
      let nivel_agregacao = '/orgao_superior';
      ...
      if (this.nivel_agregacao != '') {
        text_field = text_field.concat(` do `+ this.nivel_agregacao.toLowerCase())
        text_field = text_field.replace('_', ' ');
    ...
    analisar() {
      //console.log(Store.state.items_nivel_agregacao_selected)
  ...
  mounted() {
    ...
    this.atualizarURI();
    axios.get('/api/v1/servidor/orgao_superior', {
      params: {
        top: 27,
        mes_inicio: 1,
        ano_inicio: 2016,
  ...
  data () {
    return {
      ...
      mes_fim: 12,
      ano_fim: 2016,
      uri: '/api/v1/servidor/orgao_superior',
